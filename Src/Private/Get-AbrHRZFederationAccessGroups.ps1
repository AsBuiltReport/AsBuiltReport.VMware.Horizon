            ...
            if ($Permissions) {
                if ($InfoLevel.Settings.Administrators.FederationAccessGroup -ge 1) {
                    section -Style Heading3 "Federation Access Groups" {
                        Paragraph "The following section details the Federation Access Group information for $($HVEnvironment) server."
                        BlankLine
                        $OutObj = @()
                        ...
                        foreach ($Permission in $FilteredPermissions) {
                            #If(![string]::IsNullOrEmpty($($Permission.base.GlobalAccessGroup))){
